 ...
 {  
    return (
      ...
      <li style={{ textDecoration: todo.isCompleted ? "line-through" : "" }}>
        {todo.state ?
          <input type="text" value={todo.description} onChange={e => handleChange(todo.id,e.target.value)} autoFocus />
        ...
        <button onClick={() => handlEdit(todo.id)}>{todo.state === true ? "Update" : "Edit"}</button>
        </li>
        </FancyBorder>
    );
